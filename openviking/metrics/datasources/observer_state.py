...
class VikingDBStateDataSource(StateMetricDataSource):
    ...
    def read_vikingdb_state(self) -> ReadEnvelope[list[tuple[str, str, bool, int]]]:
        ...
        for account_id, user_id in identities:
            ...
            results.append((account_id, collection, ok, count))
            if ok_env.ok and count_env.ok:
                any_success = True
            else:
                if first_error_type is None:
                    first_error_type = ok_env.error_type or count_env.error_type
                if first_error_message is None:
                    first_error_message = ok_env.error_message or count_env.error_message
